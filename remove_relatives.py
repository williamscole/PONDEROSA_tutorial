...
def find_unrelated(degree,king_file,fam_file,pairs_file):
	def order_pair(iid1,iid2):
		return min(iid1,iid2),max(iid1,iid2)
	ibd_data = {order_pair(i.split()[1],i.split()[3]):[float(i.split()[6]),float(i.split()[7])] for i in open(king_file).readlines()[1:]}
	...
	def lda_classif(degree,pairs_df):
		...
		pairs_df["KEEP"] = pairs_df.apply(lambda x: in_sd(x.IBD1,mean_stdev[x.DEGREE]),axis=1).copy()
		pairs_df = pairs_df[pairs_df["KEEP"]]
	...
	def king_classif(degree):
		...
		return [order_pair(i.split()[1],i.split()[3]) for i in open(king_file).readlines()[1:] if i.split()[-1] in related]

	def get_coeff(iid1,iid2):
		return ibd_data.get(order_pair(iid1,iid2),[0,0])
	...
	def unrelated(iid1,iid2,degree,classif,mode):
		...
		if mode == "king":
			return not order_pair(iid1,iid2) in classif
		if mode == "fifth":
			return pihat < classif
		if mode == "lda":
	...
	def init_list(degree,classif,mode):
		sys.stdout.write("Calculating relatedness...\n")
		out_dict = {}
		for pairs in it.combinations(fam_list,r=2):
			iid1,iid2 = pairs
			if unrelated(iid1,iid2,degree,classif,mode):
				out_dict[iid1] = out_dict.get(iid1,[]) + [iid2]
				out_dict[iid2] = out_dict.get(iid2,[]) + [iid1]
		out_list = [[len(out_dict[iid]),iid] for iid in out_dict]
		out_list.sort(reverse=True)
		return out_list,out_dict

	def pick_new(cur_list,iid_list,unrel_dict):
		found = False
		to_remove = []
		for iid in iid_list:
			iid1 = iid[1]
			...
			# make sure that iid1 is unrelated to everyone
			for iid2 in cur_list:
				if iid1 == iid2:
					add = False
					continue
				if not iid1 in unrel_dict[iid2]:
					add = False
					break
			if add:
				found = True
		...
		return cur_list,[i for i in iid_list if i not in to_remove]

	def make_list(iid_list,unrelated_list,unrel_dict):
		stop = False
		while not stop:
			cur_len = len(unrelated_list)
			unrelated_list,iid_list = pick_new(unrelated_list,iid_list,unrel_dict)
	...
	def iterate_lists(degree,classif,mode):
		unrelated_len = 0
		iid_list,unrel_dict = init_list(degree,classif,mode)
		sys.stdout.write("Iterating through unrelated individuals...\n")
		master_copy = iid_list[:]
		starting_iid = [[0,None,[None]]]
		# each iteration starts with the first person in master_copy being the first iid
		while master_copy != []:
			iid = master_copy[0][1]
			num_unrelated = master_copy[0][0]
			# this means that it's impossible to make a larger set of unrelateds than already exists
			if num_unrelated < max([i[0] for i in starting_iid]):
				break
			unrelated_list = make_list(iid_list[:],[iid],unrel_dict)
			unrelated_len = len(unrelated_list)
			starting_iid.append([unrelated_len,iid,unrelated_list])
			master_copy = master_copy[1:]
			# comment out if you want to run with everyone as a starting person
			break
